...
def train_and_predict(use_existing):
    ...
    print('-'*30)
    imgs_train = np.load(out+"trainImages.npy").astype(np.float32)
    imgs_mask_train = np.load(out+"trainMasks.npy").astype(np.float32)

    imgs_test = np.load(out+"testImages.npy").astype(np.float32)
    imgs_mask_test_true = np.load(out+"testMasks.npy").astype(np.float32)
